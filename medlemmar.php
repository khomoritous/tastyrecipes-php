...
if(!isset( $_SESSION['user_id'])) {
    ...
    try {
        ...
        $mysql_dbname = 'tastyrecipes';/*** select the users name from the database ***/
        ...
        WHERE tasty_user_id = :tasty_user_id");/*** bind the parameters ***/
        $stmt->bindParam(':tasty_user_id', $_SESSION['user_id'], PDO::PARAM_INT);/*** execute the prepared statement ***/
        $stmt->execute();/*** check for a result ***/
        $phpro_username = $stmt->fetchColumn();/*** if we have no something is wrong ***/
